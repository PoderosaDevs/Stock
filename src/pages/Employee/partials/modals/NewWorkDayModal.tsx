...
export function NewWorkDayModal({ id, date }: Props) {
  ...
  const onSubmit = async (formData: AddWorkDayFormSchema) => {
    ...
    const adjustedDate = new Date(
      formData.data_trabalho.getTime() + 3 * 60 * 60 * 1000
    );
    const formattedDataTrabalho = `${adjustedDate.toISOString().slice(0, 10)} 08:00:00`;

    try {
      await FormSet({
        ...formData,
        data_trabalho: formattedDataTrabalho,
        usuarioId: id,
      });
    ...
    <Dialog.Root open={isOpen} onOpenChange={setIsOpen}>
      <Dialog.Trigger asChild>
        <button
          ...
          onClick={() => setIsOpen(true)} // Abrir o modal ao clicar
        >
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 min-w-[32rem] rounded-lg px-10 py-3 bg-gray-800 z-50">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
            <div className="flex flex-col gap-2">
              ...
              />
              {errors.data_trabalho && (
                <span className="text-red-500">{errors.data_trabalho.message}</span>
              )}
            </div>
            ...
            <div className="flex gap-4">
              <div className="flex flex-col flex-1 gap-2">
                ...
                {errors.horario_entrada && (
                  <span className="text-red-500">{errors.horario_entrada.message}</span>
                )}
              </div>
              ...
              <div className="flex flex-col flex-1 gap-2">
                ...
                {errors.horario_saida && (
                  <span className="text-red-500">{errors.horario_saida.message}</span>
                )}
              </div>
            </div>
            ...
            <div className="flex gap-4">
              ...
              <div className="flex flex-col flex-1 gap-2">
                ...
                {errors.realizados && (
                  <span className="text-red-500">{errors.realizados.message}</span>
                )}
              </div>
            </div>
